...
**Open for Marc.**
1. Walk through the Vercel preview of `ronki-app` on the phone (a Loom with voice, as in the video, is the fastest feedback), then merge draft PR #14 (https://github.com/iamyaws/Ronki/pull/14, `design/bilderbuch-app` into `main`; website and app go live together). Final preview checked on 25 Sep (late, after Marc's two calls): bundle `index-DA1vumjn.js` with the feelings entry and the 1 s guard; earlier check `index-CaYaezYE.js`: theme #0544B0, task pictures and per-egg hatch clips served.
