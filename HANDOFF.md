...
**Open for Marc.**
1. Walk through the Vercel preview of `ronki-app` on the phone (a Loom with voice, as in the video, is the fastest feedback), then merge draft PR #14 (https://github.com/iamyaws/Ronki/pull/14, `design/bilderbuch-app` into `main`; website and app go live together). Final preview checked on 25 Sep: bundle `index-CaYaezYE.js`, theme #0544B0, task pictures and per-egg hatch clips served.
...
4. Two calls from the Astra review, with options and my recommendation in `docs/reviews/2026-09-25-bilderbuch-app/response-design-r2.md`: home order (recommend a small "Wie geht's dir?" entry at the room's bottom edge) and the 4 to 5 s numbered countdown on celebration buttons (recommend a 1 s guard without numbers). The sound default stays on; Astra conceded.
